...
- **Priority**: `0` = No; `1` = [Color indices](#data-format) 1–3 of the corresponding BG/Window tile are drawn over OBJ, regardless of [OBJ priority](#byte-3--attributesflags)
...
- [LCDC bit 0](#lcdc0--bg-and-window-enablepriority)
- [OAM Attributes bit 7](#byte-3--attributesflags)
...
The [SCY and SCX](#ff42ff43--scy-scx-background-viewport-y-position-x-position)
...
[LCDC bit 5](#lcdc5--window-enable). But in Non-CGB mode this bit is only
functional as long as [LCDC bit 0](#lcdc0--bg-and-window-enablepriority) is set.
Enabling the Window makes
[Mode 3](#ppu-modes) slightly longer on scanlines where it’s visible.
(See [WX and WY](#ff4aff4b--wy-wx-window-y-position-x-position-plus-7)
