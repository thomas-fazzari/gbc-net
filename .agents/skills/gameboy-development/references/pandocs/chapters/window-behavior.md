...
These two registers specify the on-screen coordinates of [the Window](#ff4aff4b--wy-wx-window-y-position-x-position-plus-7)’s top-left pixel.
...
* For the least glitchy results, only write to `WX`, `WY`, and `LCDC` during VBlank (possibly in your [VBlank interrupt handler](interrupt-sources.md#int-40--vblank-interrupt)); if mid-frame writes are required, prefer writing during HBlank.
...
* At the beginning of each scanline, if the value of `WY` is equal to [`LY`](lcd-status-registers.md#ff44--ly-lcd-y-coordinate-read-only), the *Y condition* becomes true (and remains so for subsequent scanlines).

Note

On GBC, clearing the [Window enable bit](lcd-control.md#lcdc5--window-enable) in `LCDC` resets the *Y condition*; `WY` must be set to `LY` or greater for the Window to display again in the current frame.
...
When this counter is equal to `WX`, if the *Y condition* is true and the [Window enable bit](lcd-control.md#lcdc5--window-enable) is set in `LCDC`, background rendering is reset, beginning anew from the active row of the Window’s tilemap.
