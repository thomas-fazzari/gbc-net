...
[`stop`](reducing-power-consumption.md#using-the-stop-instruction) is often considered a **two-byte** instruction, though [the second byte is not always ignored](https://gist.github.com/SonoSooS/c0055300670d678b5ae8433e20bea595#nop-and-stop).
...
**Exception**: trying to encode `ld [hl], [hl]` instead yields [the `halt` instruction](halt.md#halt):
