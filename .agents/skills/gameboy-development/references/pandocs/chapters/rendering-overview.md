...
The most famous raster effect is modifying the [scrolling registers](viewport-position-scrolling.md#viewport-position-scrolling) between scanlines to create a [“wavy” effect](https://gbdev.io/guides/deadcscroll#effects).

A “**dot**” = one 222 Hz (≅ 4.194 MHz) time unit.
Dots remain the same regardless of whether the CPU is in [Double Speed mode](cgb-registers.md#ff4d--key1spd-cgb-mode-only-prepare-speed-switch), so there are 4 dots per Normal Speed M-cycle, and 2 per Double Speed M-cycle.
...
While the PPU is accessing some video-related memory, [that memory is inaccessible to the CPU](accessing-vram-and-oam.md#accessing-vram-and-oam) (writes are ignored, and reads return garbage values, usually $FF).
...
* **Background scrolling**: At the very beginning of Mode 3, rendering is paused for [`SCX`](viewport-position-scrolling.md#ff42ff43--scy-scx-background-viewport-y-position-x-position) % 8 dots while the same number of pixels are discarded from the leftmost tile.
...
On DMG and GBC in DMG mode, mid-scanline writes to [`BGP`](palettes.md#ff47--bgp-non-cgb-mode-only-bg-palette-data) allow observing this behavior precisely, as any delay shifts the write’s effect to the left by that many dots.
...
[^order]: Since pixels are emitted from left to right, OBJs overlapping the scanline are considered from [leftmost](object-attribute-memory-oam.md#byte-1--x-position) to rightmost, with ties broken by their index / OAM address (lowest first).
