...
Each channel generates an electronic signal; these signals are then mixed into two new channels (for stereo: one for the left ear, one for the right ear), which are then individually amplified, and then output either to the headphone jack, or the speaker[1](#footnote-speaker_mono).
...
These components are controlled by writing to the [audio registers](audio-registers.md#audio-registers).
...
The APU runs off the same Master Clock as the rest of the Game Boy, which is to say, it is fully synced with the CPU and [PPU](rendering-overview.md#rendering-overview).
...
The length of APU ticks is not affected by [CGB double speed](cgb-registers.md#ff4d--key1spd-cgb-mode-only-prepare-speed-switch), so the APU works just the same regardless of CPU speed.
...
Readers wishing to learn more should read the [APU details](audio-details.md#audio-details) chapter.

### Triggering

**Triggering** a channel causes it to turn on if it wasn’t[2](#footnote-trig_dac_off), and to start playing its wave from the beginning[3](#footnote-pulse_restart).
Most changes to a channel’s parameters take effect immediately, but some require re-triggering the channel.

### Volume & envelope

The volume can be controlled in two ways: there is a “master volume” control[4](#footnote-vol_knob) (which has separate settings for the left and right outputs), and each channel’s volume can be individually set as well (CH3’s less precisely than the others).
...
If the functionality is enabled, a channel’s **length timer** ticks up[5](#footnote-len_cnt_dir) at 256 Hz (tied to [DIV-APU](audio-details.md#div-apu)) from the value it’s initially set at.

When the length timer reaches 64 (CH1, CH2, and CH4) or 256 (CH3), the channel is turned off.

### Frequency

Music notes and audio waves are typically manipulated in terms of **frequency**[6](#footnote-pitch), i.e. how often the signal repeats per second.
However, as explained above, the Game Boy APU primarily works with durations; thus, **periods** will be used instead of frequency.[7](#footnote-len_raw)
...
1. <a id="footnote-speaker_mono"></a><a id="fr-speaker_mono-1"></a>The speaker merges back the two channels, losing the stereo aspect entirely. [↩](#fr-speaker_mono-1)
2. <a id="footnote-trig_dac_off"></a><a id="fr-trig_dac_off-1"></a>If [the channel’s DAC](audio-details.md#dacs) is off, the channel will not turn on. [↩](#fr-trig_dac_off-1)
3. <a id="footnote-pulse_restart"></a><a id="fr-pulse_restart-1"></a>Except for pulse channels, whose phase position is only ever reset by turning the APU off.
   This is usually not noticeable unless changing the duty cycle mid-note. [↩](#fr-pulse_restart-1)
4. <a id="footnote-vol_knob"></a><a id="fr-vol_knob-1"></a>This is separate from the physical volume knob located on the side of the console. [↩](#fr-vol_knob-1)
5. <a id="footnote-len_cnt_dir"></a><a id="fr-len_cnt_dir-1"></a>Internally, the length timer is inverted when written, and *that* ticks down until it reaches 0.
   But the effect is as if the counter ticked up. [↩](#fr-len_cnt_dir-1)
6. <a id="footnote-pitch"></a><a id="fr-pitch-1"></a>There is also **pitch**, which is merely a measure of how we perceive frequency.
   The higher the frequency, the higher the pitch; therefore, pitch will be omitted from the rest of the document. [↩](#fr-pitch-1)
7. <a id="footnote-len_raw"></a><a id="fr-len_raw-1"></a>Actually, the APU interfaces don’t work strictly with periods, but with values that can be thought of as *negative* periods. [↩](#fr-len_raw-1)
