...
The scroll registers are re-read on each [tile fetch](pixel-fifo.md#get-tile), except for the low 3 bits of `SCX`, which are only read at the beginning of the scanline (for the initial shifting of pixels).
