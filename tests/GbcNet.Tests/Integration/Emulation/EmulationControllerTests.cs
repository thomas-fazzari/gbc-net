...
/// <summary>
/// Provides a storage-file proxy with a name and a configurable read stream.
/// </summary>
public class TestStorageFile : DispatchProxy
{
    ...
    /// <summary>
    /// Initializes the proxy from content staged by a <see cref="Create(string, byte[])"/> overload.
    /// </summary>
    ...
    /// <summary>
    /// Creates a proxy that opens a new read-only memory stream over <paramref name="data"/>.
    /// </summary>
    public static IStorageFile Create(string name, byte[] data) =>
        Create(name, () => new MemoryStream(data, writable: false));

    /// <summary>
    /// Creates a proxy that invokes <paramref name="openRead"/> for every read request.
    /// </summary>
