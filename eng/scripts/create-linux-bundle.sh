...
app_project=${1:?Usage: create-linux-bundle.sh <app-project> <runtime>}
runtime=${2:?Usage: create-linux-bundle.sh <app-project> <runtime>}
dotnet=${DOTNET:-dotnet}

repo_root=$(CDPATH='' cd -- "$(dirname -- "$0")/../.." && pwd)
packaging_dir=$repo_root/eng/packaging/linux
...
linux-*) ;;
*)
        echo "bundle requires RUNTIME=linux-*." >&2
        exit 1
        ;;
esac
...
bundle_dir="$publish_root/$display_name.linux"
...
rsync -a "$packaging_dir/icons/hicolor/" "$bundle_dir/share/icons/hicolor/"
sed \
        ...
        "$packaging_dir/app.desktop.in" >"$bundle_dir/$executable.desktop"
chmod +x "$bundle_dir/bin/$executable"
chmod +x "$bundle_dir/$executable.desktop"
touch "$bundle_dir"
