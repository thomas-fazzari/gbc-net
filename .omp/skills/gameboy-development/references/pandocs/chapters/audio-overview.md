...
The length of APU ticks is not affected by [CGB double speed](#ff4d--key1spd-cgb-mode-only-prepare-speed-switch), so the APU works just the same regardless of CPU speed.
...
If the functionality is enabled, a channel’s **length timer** ticks up[5](#footnote-len_cnt_dir) at 256 Hz (tied to [DIV-APU](#div-apu)) from the value it’s initially set at.
...
1. The speaker merges back the two channels, losing the stereo aspect entirely. [↩](#fr-speaker_mono-1)
2. If [the channel’s DAC](#dacs) is off, the channel will not turn on. [↩](#fr-trig_dac_off-1)
...
4. This is separate from the physical volume knob located on the side of the console. [↩](#fr-vol_knob-1)
5. Internally, the length timer is inverted when written, and *that* ticks down until it reaches 0.
...
7. Actually, the APU interfaces don’t work strictly with periods, but with values that can be thought of as *negative* periods. [↩](#fr-len_raw-1)
