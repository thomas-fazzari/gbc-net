...
power down of the Game Boy or removal of the cartridge. Once the cartridge has *completely* lost power from the Game Boy, the RAM is automatically disabled to protect it.
...
`Selected ROM Bank = (Secondary Bank << 5) + ROM Bank`.[1](#footnote-MBC1M_banking)
...
(For example, a 128 KiB = 217 bytes ROM only uses bits 0–16.)
