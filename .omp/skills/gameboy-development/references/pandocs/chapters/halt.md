...
consumed](reducing-power-consumption.md#using-the-halt-instruction)) when executed. The CPU wakes up as soon as an interrupt is pending,
that is, when the bitwise AND of [`IE`](interrupts.md#ffff--ie-interrupt-enable)
and [`IF`](interrupts.md#ff0f--if-interrupt-flag) is non-zero.

Most commonly, [`IME`](interrupts.md#ime-interrupt-master-enable-flag-write-only) is
set. In this case, the CPU simply wakes up, and before executing the instruction
after the `halt`, the [interrupt handler is called](interrupts.md#interrupt-handling)
normally.
