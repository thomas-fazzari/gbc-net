...
* **Audio on/off** (*Read/Write*): This controls whether the APU is powered on at all (akin to [`LCDC` bit 7](lcd-control.md#lcdc7--lcd-enable)).
  ...
  Turning the APU off, however, does not affect [Wave RAM](#ff30ff3f--wave-pattern-ram), which can always be read/written, nor the [DIV-APU](audio-details.md#div-apu) counter.
* **CHn on?** (*Read-only*): Each of these four bits allows checking whether channels are active[2](#footnote-nr52_dac).
  ...
  * The channel’s [length timer](audio-overview.md#length-timer) is enabled in `NRx4` and expires, or
  * *For CH1 only*: when the [period sweep](#ff10--nr10-channel-1-sweep) overflows[4](#footnote-freq_sweep_underflow), or
  * [The channel’s DAC](audio-details.md#dacs) is turned off.
...
Note that selecting or de-selecting a channel whose [DAC](audio-details.md#dacs) is enabled will [cause an audio pop](audio-details.md#mixer).
...
* **Pace**: This dictates how often sweep “iterations” happen, in units of 128 Hz ticks[5](#footnote-div_apu) (7.8 ms).
  Note that the value written to this field is not re-read by the hardware until a sweep iteration completes, or the channel is [(re)triggered](audio-overview.md#triggering).
...
This register controls both the channel’s [length timer](audio-overview.md#length-timer) and [duty cycle](https://en.wikipedia.org/wiki/Duty_cycle) (the ratio of the time spent low vs. high).
...
* **Initial length timer** (*Write-only*): The higher this field is, [the shorter the time before the channel is cut](audio-overview.md#length-timer).
...
Setting bits 3-7 of this register all to 0 (initial volume = 0, envelope = decreasing) turns the DAC off (and thus, the channel as well), which [may cause an audio pop](audio-details.md#mixer).
...
This register stores the low 8 bits of the channel’s 11-bit “[period value](audio-overview.md#frequency)”.
...
* **Trigger** (*Write-only*): Writing any value to `NR14` with this bit set [triggers](audio-overview.md#triggering) the channel, causing the
  following to occur:
  ...
  * Sweep does [several things](audio-details.md#pulse-channel-with-sweep-ch1).
* **[Length](audio-overview.md#length-timer) enable** (*Read/Write*): Takes effect immediately upon writing to this register.
...
This register controls CH3’s [DAC](audio-details.md#dacs).
...
Turning the DAC off [may cause an audio pop](audio-details.md#mixer).

### FF1B — NR31: Channel 3 length timer [write-only]

This register controls the channel’s [length timer](audio-overview.md#length-timer).

| | 7 | 6 | 5 | 4 | 3 | 2 | 1 | 0 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **NR31** | Initial length timer | | | | | | | |

The higher the [length timer](audio-overview.md#length-timer), the shorter the time before the channel is cut.
...
* **Trigger** (*Write-only*): Writing any value to `NR34` with this bit set [triggers](audio-overview.md#triggering) the channel, causing the
  following to occur:
...
This is especially relevant on GBA, whose [mixer behaves as if DACs are always enabled](audio-details.md#game-boy-advance-audio).
...
* **[LFSR](audio-details.md#noise-channel-ch4) width**: `0` = 15-bit, `1` = 7-bit (more regular output; some frequencies sound more like pulse than noise).

  LFSR lockup

  Switching from 15- to 7-bit mode when the LFSR [is in a certain state](audio-details.md#noise-channel-ch4) can “lock it up”, which essentially silences CH4; this can be avoided by retriggering CH4, which resets the LFSR.
...
* **Trigger** (*Write-only*): Writing any value to `NR44` with this bit set [triggers](audio-overview.md#triggering) the channel, causing the
  following to occur:
  ...
  * [LFSR bits](audio-details.md#noise-channel-ch4) are reset.
* **[Length](audio-overview.md#length-timer) enable** (*Read/Write*): Takes effect immediately upon writing to this register.

---

1. <a id="footnote-dmg_apu_off"></a><a id="fr-dmg_apu_off-1"></a>…and the length timers (in `NRx1`) on monochrome models. [↩](#fr-dmg_apu_off-1)
2. <a id="footnote-nr52_dac"></a><a id="fr-nr52_dac-1"></a>Actually, only the status of the channels’ *generation* circuits is reported, not the status of [the DACs](audio-details.md#dacs). A channel can only be ON if its corresponding DAC is, though. [↩](#fr-nr52_dac-1)
3. <a id="footnote-dac_off"></a><a id="fr-dac_off-1"></a>If [the channel’s DAC](audio-details.md#dacs) is off, then the write to NRx4 will be ineffective and won’t turn the channel on. [↩](#fr-dac_off-1)
4. <a id="footnote-freq_sweep_underflow"></a><a id="fr-freq_sweep_underflow-1"></a>The period sweep cannot normally underflow, so a “decreasing” sweep (`NR10` bit 3 set) cannot turn the channel off. [↩](#fr-freq_sweep_underflow-1)
5. <a id="footnote-div_apu"></a><a id="fr-div_apu-1"></a>[As long as `DIV` is not written to](audio-details.md#div-apu). [↩](#fr-div_apu-1)
6. <a id="footnote-wave_access"></a><a id="fr-wave_access-1"></a>The way it works is that wave RAM is a 16-byte memory buffer, and while it’s playing, CH3 has priority over the CPU when choosing which of those 16 bytes is accessed.
   So, from the CPU’s point of view, wave RAM reads out the same byte, regardless of the address. [↩](#fr-wave_access-1)
7. <a id="footnote-not_white"></a><a id="fr-not_white-1"></a>By default, the noise will sound close to white; but it can be manipulated to sound differently. [↩](#fr-not_white-1)
