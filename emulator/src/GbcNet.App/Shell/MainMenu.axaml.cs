...
internal sealed partial class MainMenu : UserControl
{
    ...
    public MainMenu()
    {
        ...
        PauseCommand = CreateCommand(
            _ => TogglePause?.Invoke(),
            () => _pauseEnabled,
            closesMenu: false
        );
        ...
        MuteCommand = CreateCommand(_ => ToggleMute?.Invoke(), closesMenu: false);
        ...
        FastForwardCommand = CreateCommand(_ => ToggleFastForward?.Invoke(), closesMenu: false);
        FastForwardSpeedCommand = CreateCommand(
            parameter =>
            {
    ...
    public void SetStateSlotDates(IReadOnlyList<DateTime?> dates)
    {
        ...
        for (var slotIndex = 0; slotIndex < StateSlotCount; slotIndex++)
        {
            ...
            var label = date is not null
                ? $"Slot {slotIndex + 1} — {date:g}"
                : $"Slot {slotIndex + 1}";

            if (!Equals(StateSlotComboBox.Items[slotIndex], label))
            {
                StateSlotComboBox.Items[slotIndex] = label;
            }
        }

        if (selectedIndex >= 0 && StateSlotComboBox.SelectedIndex != selectedIndex)
        {
            StateSlotComboBox.SelectedIndex = selectedIndex;
        }
    ...
    {
        FastForwardButton.IsChecked = isEnabled;
    ...
    public void SetMuteState(bool isMuted) => MuteButton.IsChecked = isMuted;

    private MenuCommand CreateCommand(
        Action<object?> execute,
        Func<bool>? canExecute = null,
        bool closesMenu = true
    )
    {
    ...
    private void ConfigurePanel()
    {
        for (var slotIndex = 0; slotIndex < StateSlotCount; slotIndex++)
        {
            StateSlotComboBox.Items.Add($"Slot {slotIndex + 1}");
        }
